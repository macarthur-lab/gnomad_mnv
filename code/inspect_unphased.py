...
exomes = get_gnomad_data("genomes", release_samples=True, adj=True, release_annotations=True)#実際はgenomeだけど.
...
ex20 = hl.filter_alleles(ex20, lambda allele, i: hl.is_snp(ex20.alleles[0], allele))  # currently take only SNP
ex20 = ex20.filter_entries(ex20.GT.is_het())  # throw away unwanted entries (non alt)
ex20_pair = hl.window_by_locus(ex20, 2) #just look at nearby pairs for now
ex20_pair = ex20_pair.filter_entries((hl.is_defined(ex20_pair.GT) & (ex20_pair.prev_entries.length() > 0)))
ex20_pair = ex20_pair.filter_entries(ex20_pair.prev_entries.filter(lambda x: x.GT.is_het()).length() > 0)
...
et = et.annotate(agrees_PID=((et.GT.phased) & (et.prev_entry.GT.phased) &(et.PID == et.prev_entry.PID) & hl.is_defined(et.PID)))
et = et.annotate(dist=et.locus.position - et.prev_row.locus.position) #annotating the distance
et = et.filter(et.dist>0)
#et.show()
et.select("agrees_PID","info_DP").write("gs://gnomad-qingbowang/gen_chr20_all_siteDP.ht", overwrite=True)
...
exomes = get_gnomad_data("exomes", release_samples=True, adj=True, release_annotations=True)#実際はgenomeだけど.
#ex20 = hl.filter_intervals(exomes.select_rows("allele_info").select_cols(), [hl.parse_locus_interval("20:start-2M")]) #first 2Mb
#ex20 = hl.filter_intervals(exomes.select_rows("info_DP").select_cols(), [hl.parse_locus_interval("20")])
ex20 = exomes.select_rows("info_DP").select_cols()
ex20 = ex20.filter_cols(hl.rand_bool(0.01)) #~=1500 samples for small test, exome
#ex20 = ex20.filter_cols(hl.rand_bool(0.1)) #~=1500 samples for small test, genome
#全sampleに関して行うとしよう.
ex20 = hl.filter_alleles(ex20, lambda allele, i: hl.is_snp(ex20.alleles[0], allele))  # currently take only SNP
ex20 = ex20.filter_entries(ex20.GT.is_het())  # throw away unwanted entries (non alt)
...
et = et.annotate(agrees_PID=((et.GT.phased) & (et.prev_entry.GT.phased) &(et.PID == et.prev_entry.PID) & hl.is_defined(et.PID)))
#et.show()
et.select("agrees_PID","info_DP").write("gs://gnomad-qingbowang/ex_chr_all_siteDP.ht", overwrite=True)
print (et.count()) #sample size
